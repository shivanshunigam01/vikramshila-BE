...
const dateFilter = (filter) => {
  const now = new Date();
  let from;
  if (filter === "week") { from = new Date(now); from.setDate(now.getDate() - 7); }
  else if (filter === "month") { from = new Date(now); from.setMonth(now.getMonth() - 1); }
  else if (filter === "year") { from = new Date(now); from.setFullYear(now.getFullYear() - 1); }
...
exports.markContacted = async (req, res) => {
  const item = await Enquiry.findByIdAndUpdate(req.params.id, { contacted: true }, { new: true });
