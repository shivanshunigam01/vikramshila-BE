    ...
    load(context) {
        return {
            transform: (res) => {
                ...
                let error;
                if (!context.response) {
                    error = new api_errors_1.EmptyResponseError('Fail to Fetch', errorContext);
                }
                    ...
                    if (context.response.status !== 204
                        ...
                        error = new api_errors_1.EmptyResponseError(context.response.statusText, errorContext);
                    }
                }
                if (error) {
                    if (this.callback) {
